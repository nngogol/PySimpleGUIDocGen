from PyQt5 import QtCore, QtGui, QtWidgets; from PyQt5.QtCore import *; from PyQt5.QtWidgets import *; from PyQt5.QtGui import *
import datetime
import random

def getv2(selfa, names):
	ui = getattr(selfa, 'ui')
	return [ (i, getv(getattr(ui, i))) for i in names.split(' ')]
...
def setv(widget, val):
	...
	# date
		...
		raise Exception(f'{val} - value is wrong for {type_w}!')
			...
			# dd = r.date()
			# tt = r.time()
			# qtt = QTime(tt.hour, tt.minute, tt.second)
			# qdd = QDate(dd.year, dd.month, dd.day)
