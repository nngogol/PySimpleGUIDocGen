...
def main(do_full_readme=False, files_to_include:list=[], logger=None, output_name='FINALreadme.md', delete_html_comments=True):
	"""
	Goal is:
	1) load 1_ 2_ 3_
	...
	Release_notes 			= readfile('3_release_notes.md') 	# 3
	
	# ▓▓▓▓▓▓▓▓▓▓▓▓▓▓
	# ▓▓▓▓▓▓▓▓▓▓▓▓ 2 GET classes, funcions, varialbe a.k.a. memes
	# ▓▓▓▓▓▓▓▓▓▓▓▓▓▓
	psg_members = getmembers(PySimpleGUIWeblib)
	...
	injection_points, classes_method_tags = [], [ j for j in mark_points if 'func.' not in j]
	...
	for tag in func_tags:
		...
		try:
			...
			injection_points.append({
				"tag" : tag, "function_object" : founded_function[0],
				"parent_class" : None, "part1" : 'func', "part2" : part2, "number" : number,
			})
	...
	for tag in classes_method_tags:
		try:
			...
			# ░▒▓ find method ▓▒░
			try:
				founded_method = getattr(founded_class[0], method_name) if method_name != 'doc' else None
			...
			# ░▒▓ collect ▓▒░
			injection_points.append({
				"tag" : tag, "function_object" : founded_method, "parent_class" : founded_class[0],
				"part1" : part1, "part2" : part2, "number" : number,
			})
		except Exception as e:
			if logger: logger.error(f'```````````````````````{str(e)}')
			continue
	...
	files = []
	if 0 in files_to_include: files.append(readme)
	if 1 in files_to_include: files.append(Release_notes)
